...
private:
    float _reach_time;
    float _integral_rad;
    float _const_cycle_time;
    float _cycle_time;
    float _hold_data;
    float _target_data;
    float _bef_target_data;
    float _data_delta;
    float _addition_data;
    float _output;
    bool _is_init;
...
public:
    ...
    void init(float REACH_TIME, float CONST_CYCLE_TIME){
    ...
    void Set_Reach_Time(float TIME){
        _reach_time = TIME;
    }

    void Set_Data(float TARGET){
        _target_data = TARGET;
    }

    void Update(float CYCLE_TIME = -1){
        if(!_is_init){
            return;
        }
    ...
    float Cal_Data(float TARGET, float CYCLE_TIME = -1){
